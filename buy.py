...
document.addEventListener("DOMContentLoaded", function () {
    let savedAddress = localStorage.getItem("selected_address");
    if (savedAddress) {
...
<script>
document.addEventListener("DOMContentLoaded", function () {
    // Check if an address is already saved in localStorage
    let savedAddress = localStorage.getItem("selected_address");
    if (savedAddress) {
        document.querySelector("input[name='address']").value = savedAddress;
    }
    ...
    document.querySelectorAll('.address-option').forEach(option => {
        option.addEventListener('click', function() {
            let selectedAddress = this.getAttribute('data-address');
            localStorage.setItem('selected_address', selectedAddress);
            document.querySelector("input[name='address']").value = selectedAddress;
        });
    });
});
</script>
...
p = "SELECT * FROM address WHERE user_id=%s"
cur.execute(p, (uid,))
h = cur.fetchall()
...
if h:
    ...
    <div class="card p-3">
        ...
        <form action="#" method="POST">
            ...
            <button type="button" class="address-option btn btn-primary" data-address="{b[3]}">Select This Address</button>
        </form>
...
print("""
     <script>
        fetch(`/cgi-bin/get_address_data.py?id=${uid}`)
            ...
            .then(data => {
                if (data.length > 0) {
                    ...
                    document.getElementById("pincode").textContent = data[0].pincode;
...
b = "SELECT * FROM products WHERE product_id=%s"
cur.execute(b, (product_id,))
            ...
            if selected_address:
                ...
                if not all([user_id, address, product_details, total, quantity, payment_method]):
                    ...
                    if payment_method == "Online Payment":
                        # Redirect to another page for online payment
                        print(f"<script>window.location.href = 'payment.py?id={user_id}';</script>")
                    elif payment_method == "Cash On Delivery":
                        # Redirect to the home page
                        print(f"<script>window.location.href = 'homelog.py?id={user_id}';</script>")
